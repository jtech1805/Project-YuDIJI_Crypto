...
| ADR-062 | Embedding requests carry explicit purpose and versioned normalization is a provider-neutral authority | ACCEPTED | Track C-B2A | Freezes request-level document/query purpose, exact provider-neutral normalization authority, L2 canonical vectors, transient raw vectors, and purpose-bearing vector digests before Gemini embedding implementation. | None | None |
| ADR-063 | Vector index publication uses immutable provider projections separate from canonical embeddings | ACCEPTED | Track C-B3A | Separates canonical embedding authority from append-only vector-index publication projections with exact entry, index, namespace, digest, metadata, and multi-index lineage. | None | None |
| ADR-064 | Current User role authority and composed caller cancellation | ACCEPTED | Track C-E0 | Makes current User roles authoritative for USER/INTERNAL/ADMIN access, keeps JWTs identity-only, and composes caller cancellation with the governed runtime deadline. | None | None |
