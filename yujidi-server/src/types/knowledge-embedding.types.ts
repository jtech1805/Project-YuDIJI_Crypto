...
import type {
  KnowledgeChunkSetManifestIdentity,
  KnowledgeChunkSetStrategyIdentity,
} from "./knowledge-chunk-set-manifest.types.js";
import type {
  KnowledgeCorpus,
...
export const KNOWLEDGE_EMBEDDING_PURPOSES = [
  "RETRIEVAL_DOCUMENT",
  "RETRIEVAL_QUERY",
] as const;
export type KnowledgeEmbeddingPurpose =
  (typeof KNOWLEDGE_EMBEDDING_PURPOSES)[number];

export const KNOWLEDGE_SIMILARITY_METRICS = [
  "COSINE",
  "DOT_PRODUCT",
  "EUCLIDEAN",
] as const;
export type KnowledgeSimilarityMetric =
  (typeof KNOWLEDGE_SIMILARITY_METRICS)[number];
...
export type KnowledgeEmbeddingSchemaDefinition =
...
export const KNOWLEDGE_EMBEDDING_GENERATION_POLICY: KnowledgeEmbeddingGenerationPolicy =
  Object.freeze({
...
export type KnowledgeEmbeddingCommand = Readonly<{
  ...
  embeddingTextProjector: Readonly<{
    projectorId: string;
    projectorVersion: number;
  }>;
  ...
  normalizationStrategy: Readonly<{
    normalizationStrategyId: string;
    normalizationStrategyVersion: number;
  }>;
  purpose: KnowledgeEmbeddingPurpose;
...
export type PersistedKnowledgeEmbedding = KnowledgeEmbeddingCommand &
...
export type KnowledgeEmbeddingReadResult =
  ...
  | Readonly<{
      found: false;
      code: "NOT_FOUND" | "PERSISTENCE_FAILED" | "INVARIANT_VIOLATION";
    }>;
