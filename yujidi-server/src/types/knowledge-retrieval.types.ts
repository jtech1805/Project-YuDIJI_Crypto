...
} from "./knowledge-chunk.types.js";
import type {
  KnowledgeEmbeddingIdentity,
  KnowledgeEmbeddingSchemaIdentity,
  KnowledgeSimilarityMetric,
} from "./knowledge-embedding.types.js";
import type {
  KnowledgeCorpus,
...
export type KnowledgeRetrievalPolicyIdentity = Readonly<{
  policyId: string;
  policyVersion: number;
}>;
export type KnowledgeScoreRange = Readonly<{
  minimum: number;
  maximum: number;
  clamp: boolean;
}>;
export type KnowledgeRetrievalPolicy = KnowledgeRetrievalPolicyIdentity &
  Readonly<{
    allowedCorpora: readonly KnowledgeCorpus[];
    allowedTrustLevels: readonly KnowledgeTrustLevel[];
    maxQueryCharacters: number;
    maxQueryConcepts: number;
    maxEligibleDocuments: number;
    vectorCandidateLimit: number;
    lexicalCandidateLimit: number;
    finalTopK: number;
    vectorWeight: number;
    lexicalWeight: number;
    metadataMatchWeight: number;
    trustWeight: number;
    vectorScoreRange: KnowledgeScoreRange;
    lexicalScoreRange: KnowledgeScoreRange;
    trustScores: Readonly<Partial<Record<KnowledgeTrustLevel, number>>>;
    maxChunksPerDocument: number;
    maxParentChunks: number;
    maxSiblingChunks: number;
    contextCharacterBudget: number;
    maxPassageCharacters: number;
    includeVectorSearch: boolean;
    includeLexicalSearch: boolean;
    includeParentContext: boolean;
    includeSiblingContext: boolean;
    excludeSupersededMembers: boolean;
    vectorFailureFallback: "FAIL" | "LEXICAL_ONLY";
    lexicalFailureFallback: "FAIL" | "VECTOR_ONLY";
...
export type KnowledgeRetrievalRequest = Readonly<{
  ...
  scope: Readonly<{
    ...
    trustLevels: readonly KnowledgeTrustLevel[];
    documentTypes?: readonly PlatformKnowledgeDocumentType[];
  }>;
  filters?: KnowledgeRetrievalFilters;
  embeddingSchema: KnowledgeEmbeddingSchemaIdentity;
  vectorIndex: KnowledgeVectorIndexDefinitionIdentity;
  retrievalPolicy: KnowledgeRetrievalPolicyIdentity;
  eligibleDocuments: readonly KnowledgeDocumentIdentity[];
  asOf: Date;
}>;
export type KnowledgeQueryTextProjection = Readonly<{
  projectorId: string;
...
export type KnowledgeVectorSearchRequest = Readonly<{
  index: KnowledgeVectorIndexDefinitionIdentity;
  namespace: string;
  indexSchema: Readonly<{ indexSchemaId: string; indexSchemaVersion: number }>;
  asOf: Date;
  queryVector: readonly number[];
  vectorDimension: number;
  metric: KnowledgeSimilarityMetric;
...
export type KnowledgeLexicalSearchDocument = Readonly<{
  documentId: string;
...
export type KnowledgeLexicalSearchChunk = Readonly<{
  document: KnowledgeLexicalSearchDocument;
  chunkIdentity: KnowledgeChunkIdentity;
  content: string;
...
export type KnowledgeUntrustedLexicalCandidate = Readonly<{
  documentIdentity: KnowledgeDocumentIdentity;
  chunkIdentity: KnowledgeChunkIdentity;
  score: number;
  matchedFilters: readonly string[];
}>;
export type KnowledgeLexicalSearchResult = Readonly<{
  status: "COMPLETED" | "NO_CANDIDATES" | "VALIDATION_FAILED" | "SEARCH_FAILED";
  candidates: readonly KnowledgeUntrustedLexicalCandidate[];
  failureCode?: string;
}>;
...
export type KnowledgeCandidateSource =
  | Readonly<{
      ...
      embeddingIdentity: KnowledgeEmbeddingIdentity;
      chunkSetIdentity: KnowledgeChunkSetManifestIdentity;
      indexedChunkDigest: string;
      indexedVectorDigest: string;
      searchableMetadata?: KnowledgeVectorSearchableMetadata;
    }>
  | Readonly<{ type: "LEXICAL"; rawScore: number }>;
export type KnowledgeRetrievalCandidate = Readonly<{
  documentIdentity: KnowledgeDocumentIdentity;
  chunkIdentity: KnowledgeChunkIdentity;
  sources: readonly KnowledgeCandidateSource[];
  matchedFilters: readonly string[];
}>;
export const KNOWLEDGE_RETRIEVAL_EXCLUSION_CODES = [
  ...
  "CHUNK_LINEAGE_MISMATCH",
  "SEARCHABLE_METADATA_MISMATCH",
  "DOCUMENT_NOT_ELIGIBLE",
  "DOCUMENT_NOT_FOUND",
  "DOCUMENT_OUTSIDE_EFFECTIVE_TIME",
  "DOCUMENT_SUPERSEDED",
  "MANIFEST_NOT_FOUND",
...
export type KnowledgeRetrievalExclusionCode =
  (typeof KNOWLEDGE_RETRIEVAL_EXCLUSION_CODES)[number];
export type KnowledgeRetrievalExclusion = Readonly<{
  code: KnowledgeRetrievalExclusionCode;
  documentIdentity: KnowledgeDocumentIdentity;
  chunkIdentity?: KnowledgeChunkIdentity;
  sourceTypes: readonly ("VECTOR" | "LEXICAL")[];
}>;
export type ValidatedKnowledgeRetrievalCandidate = Readonly<{
  candidate: KnowledgeRetrievalCandidate;
  document: import("./knowledge-document.types.js").PersistedKnowledgeDocument;
  chunk: import("./knowledge-chunk.types.js").PersistedKnowledgeChunk;
  verifiedChunks: readonly import("./knowledge-chunk.types.js").PersistedKnowledgeChunk[];
  chunkSetIdentity: import("./knowledge-chunk-set-manifest.types.js").KnowledgeChunkSetManifestIdentity;
}>;
export type RankedKnowledgeRetrievalCandidate =
...
export type KnowledgeRetrievalResult = Readonly<{
  status:
    ...
    | "FEATURE_DISABLED"
    | "VALIDATION_FAILED"
    | "EMBEDDING_SCHEMA_NOT_FOUND"
    | "INDEX_DEFINITION_NOT_FOUND"
    | "INDEX_INELIGIBLE"
    ...
    | "CITATION_ASSEMBLY_FAILED"
    | "INVARIANT_VIOLATION";
  context:
    | import("./knowledge-context.types.js").KnowledgeRetrievalContext
    | null;
  failureCode?: string;
  summary: Readonly<{
