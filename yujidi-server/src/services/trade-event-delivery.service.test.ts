...
} from "./trade-event-delivery.service.js";
import { TradeEventService, type TradeEventRecord } from "./trade-event.service.js";
...
test("existing market alert websocket event name remains NEW_ALERT", async () => {
  ...
  const analyzerSource = await source.readFile(
    new URL("./analyzer.service.ts", import.meta.url),
    "utf8",
  );
