...
export class TradePlanService {
  ...
  public async getTradePlanDashboardSummary(userId: string, planId: string): Promise<Record<string, unknown>> {
    ...
    const totals = this.calculateDashboardTotals(plan, capitalAdjustments, tradeResults, openActiveTrades);
    ...
    const blockReasons = this.getDashboardBlockReasons({
      plan,
      remainingTrades,
      riskMode,
      dailyStopTradingTriggered: dailyRiskState?.stopTradingTriggered === true,
    });
    const stopTradingReasons = this.getStopTradingReasons(riskMode, dailyRiskState);
    ...
    const totalRealizedR = tradeResults.reduce((sum, result) => sum + this.safeNumber(result.realizedR), 0);
    ...
    return {
      ...
      risk: {
        riskMode,
        dailyRiskUsed: this.safeNumber(dailyRiskState?.netPnl ?? dailyRiskState?.grossPnl),
        planRiskUsed: this.safeNumber(riskState?.netPnl ?? riskState?.grossPnl),
      ...
      latestTrades: tradeResults.slice(0, 10).map((result) => ({
  ...
  public async resetRiskLock(
    ...
    const previousStopReasons = [
      ...this.getStopTradingReasons(existingRiskState?.riskMode ?? "NORMAL_RISK", null),
      ...dailyStates.flatMap((state) => this.getStopTradingReasons(state.riskMode ?? "NORMAL_RISK", state)),
    ];
  ...
  private async getTradeUsageSnapshot(userId: string, planId: string): Promise<TradeUsageSnapshot> {
    ...
    const riskStateTotalTrades = this.safeNumber(riskState?.totalTrades);
  ...
  private calculateDashboardTotals(
    ...
    const realizedGrossPnl = tradeResults.reduce((sum, result) => sum + this.safeNumber(result.grossPnl), 0);
    const realizedNetPnl = tradeResults.reduce((sum, result) => {
      return sum + this.safeNumber(result.netPnl ?? result.grossPnl);
    }, 0);
    ...
    const openRiskAmount = openActiveTrades.reduce((sum, trade) => sum + this.safeNumber(trade.actualRiskAmount), 0);
  ...
  private getDashboardBlockReasons(input: {
    plan: TradePlanRecord;
    ...
    if (input.plan.status !== "ACTIVE") {
      blockReasons.push("TRADE_PLAN_NOT_ACTIVE");
    }
    if (input.riskMode === "STOP_TRADING") {
      blockReasons.push("STOP_TRADING_ACTIVE");
    }
    if (input.dailyStopTradingTriggered) {
      blockReasons.push("DAILY_STOP_TRADING_ACTIVE");
    }
  ...
  private getStopTradingReasons(
    riskMode: RiskMode,
    dailyRiskState: DashboardDailyRiskState | null,
  ): string[] {
    const reasons: string[] = [];
    if (riskMode === "STOP_TRADING") {
      reasons.push("Plan risk limit reached.");
    }
    if (dailyRiskState?.dailyLossLimitHit === true) {
    ...
    return reasons.length > 0 ? reasons : riskMode === "STOP_TRADING" ? ["Plan risk limit reached."] : [];
  }
