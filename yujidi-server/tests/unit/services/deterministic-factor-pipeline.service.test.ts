...
import { DeterministicFactorPipelineService } from "../../../src/services/scoring/deterministic-factor-pipeline.service.js";
...
test("composition source delegates without duplicated phase logic or forbidden imports", () => { const source = readFileSync(new URL("../../../src/services/scoring/deterministic-factor-pipeline.service.ts", import.meta.url), "utf8"); for (const word of ["factor-input-assembly.service", "registry", "evidence-read", "evidence.repository", "provider", "scoring-engine", "controller", "scheduler", "Math.round", "toFixed", "aggregatePoints +=", "normalizedScore =", ".filter((band"]) assert.equal(source.toLowerCase().includes(word.toLowerCase()), false, word); });
