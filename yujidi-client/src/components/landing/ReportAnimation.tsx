    ...
    <div ref={ref} className="glass rounded-2xl overflow-hidden">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 px-6 py-4 border-b border-border bg-card/40">
        <div>
          ...
          <div className="text-xl font-semibold mt-0.5">Detected Pattern · #YJD-04219</div>
        </div>
        <div className="flex items-center gap-6 font-mono text-xs">
          ...
          <div>
            <div className="text-muted-foreground text-[10px] tracking-widest">VOLATILITY</div>
      ...
      <div className="grid lg:grid-cols-3 gap-px bg-border/60">
        ...
        <div className="bg-card/40 p-6 space-y-5">
          <div>
            <div className="font-mono text-[10px] text-muted-foreground tracking-widest mb-2">TECHNICAL LEVELS</div>
            <div className="space-y-2 font-mono text-sm">
              <Row label="Buy-side Liquidity" value="$68,400" tone="muted" />
              <Row label="Sell-side Pressure" value="$65,200" tone="muted" />
              <Row label="Model Projection" value="$63,820" tone="primary" icon={<Target className="h-3 w-3" />} />
            </div>
          </div>

          <div>
            <div className="font-mono text-[10px] text-muted-foreground tracking-widest mb-2">DETECTED PATTERNS</div>
            <div className="space-y-2">
              <Pill label="Sell-side pressure · Binance" tone="bear" />
          ...
          <div className="pt-2 border-t border-border">
            <div className="flex items-center gap-2 font-mono text-[10px] text-primary tracking-widest mb-2">
              <Activity className="h-3 w-3" /> MODEL CONFIDENCE
            </div>
      ...
      <div className="px-6 py-3 border-t border-border bg-card/40 font-mono text-[10px] text-muted-foreground leading-relaxed">
        Market data sourced via Public Exchange APIs. AI insights are experimental and subject to model hallucinations. Not investment advice.
      </div>
    </div>
  );
};
...
const Pill = ({ label, tone }: { label: string; tone: "bear" | "muted" }) => (
  <div
    className={`px-3 py-1.5 rounded-md font-mono text-xs border ${tone === "bear" ? "bg-bear/10 border-bear/30 text-bear" : "bg-secondary border-border text-muted-foreground"
      }`}
  >
    {label}
  </div>
